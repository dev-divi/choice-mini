...
PLATFORM_FEES = {
    "tiktok_shop": {
        "commission": settings.TIKTOK_COMMISSION_RATE,   # 6%
        "payment": settings.TIKTOK_PAYMENT_FEE,          # 2.9%
        "fixed": settings.TIKTOK_FIXED_FEE,              # $0.30
        "affiliate": settings.TIKTOK_AFFILIATE_COMMISSION,  # 20%
    },
    "shopify": {
        "commission": 0.0,     # No platform commission
        "payment": 0.029,     # Shopify Payments 2.9%
        "fixed": 0.30,        # $0.30 per transaction
        "affiliate": 0.0,     # No built-in affiliate program
    },
}
...
class LaunchAgent:
    ...
    def calculate_price(self, supplier: SupplierMatch) -> tuple[float, float, float]:
        ...
        """
        fees = PLATFORM_FEES[self.platform]
